...
def blueprint_tool(event):
    ...
    global bluespot, bluespot_counter
...
def blueprint_select_cb():
    ...
    canvas = tk.Canvas(blueprint_frame, width = 500, height = 500)
...
def image_select_cb(): # 'image selection' button callback function 
    ...
    button.
    '''
    global canvas, f, file_counter, master_frame

    master_frame = tk.Frame(parking_app, relief = SUNKEN, width = 1000, height = 1000)
    ...
    image_frame.pack(side = LEFT, expand=YES)
    ...
    tk.Label(frame, text = "Send Data: ").grid(row = 1, column = 4,sticky = W,  pady = 20, padx = 10)
    tk.Button(frame, text='Finish', command=finish_cb).grid(row=1, column=5, sticky = W, pady = 20, padx = 5)
    ...
    tools_frame.pack(side = RIGHT, padx=2)

    parking_app.eval('tk::PlaceWindow %s center' % parking_app.winfo_toplevel())

# set up initial GUI labels and buttons. This includes 'image select' label/button and company logo (eventually)
